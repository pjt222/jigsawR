...
# =============================================================================
# Load extracted modules (#108)
# =============================================================================
source("modules/ui_helpers.R", local = TRUE)
source("modules/download_module.R", local = TRUE)
          ...
          accordion(
            ...
            accordion_panel(
              ...
              radioButtons("puzzle_type", "Puzzle Type:",
                          ...
                          inline = TRUE),
              uiOutput("puzzle_type_description")
            ),
            ...
            accordion_panel(
              ...
              helpText(
                "PILES = Puzzle Input Line Entry System, inspired by",
                tags$a("SMILES", href = "https://en.wikipedia.org/wiki/Simplified_molecular-input_line-entry_system", target = "_blank"),
                "notation from chemistry.",
                "Example: '1-2-3' fuses pieces 1, 2, and 3."
              )
            ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$generate, {
    ...
    tryCatch({
      ...
      showNotification(friendly_error_message(e$message), type = "error", duration = 10)
    })
  })
  ...
  observe({
    ...
    tryCatch({
      ...
      showNotification(friendly_error_message(e$message), type = "error", duration = 5)
    })
  })

  # Puzzle type description (#117)
  output$puzzle_type_description <- renderUI({
    desc <- switch(input$puzzle_type,
      "rectangular" = "Classic grid-based jigsaw with interlocking tabs",
      "hexagonal" = "Hexagonal tiling with optional warping and truncation",
  ...
  rendered_svg <- reactive({
    ...
    svg <- tryCatch({
      ...
      paste0(
        ...
        '<text x="50" y="150" fill="red">Error: ', htmltools::htmlEscape(friendly_error_message(e$message)), '</text>',
        '</svg>'
      )
    })
  ...
  # Reactive values bundle for download module (#108)
  rv <- list(
    puzzle_data = puzzle_data,
    svg_content = svg_content,
    positioned_result = positioned_result
  )

  # Register download handlers (#108)
  register_download_handlers(input, output, session, rv, build_puzzle_params)
}
